...
public class Enemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!dead)
        {
            ...
            if ( attacking > 0)
            {
                ...
                if (attacking < 0.5 && attackTarget != null)
                {
        ...
        {
            Vector3 zombiePosition = Camera.main.WorldToScreenPoint(transform.position);
            GetComponent<AIPath>().enabled = false;
            if (zombiePosition.z < 0)
            {
                Destroy(gameObject);
            }
        }
       ...
       if (!dead)
        {
            ...
            if (health <= 0)
            {
                ...
                DropXP();
                GameManager.monsterType zombie = GameManager.monsterType.zombie;
                GameManager.instance.add_kill(zombie);
    ...
    void OnTriggerEnter(Collider other){
        ...
        if (other.transform.tag == "Player" && !dead && other.GetType() == typeof(CapsuleCollider))
        {
